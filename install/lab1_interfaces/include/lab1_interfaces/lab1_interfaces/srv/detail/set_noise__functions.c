  ...
  lab1_interfaces__srv__SetNoise_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    lab1_interfaces__srv__SetNoise_Request * data =
      (lab1_interfaces__srv__SetNoise_Request *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!lab1_interfaces__srv__SetNoise_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          lab1_interfaces__srv__SetNoise_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
  ...
  lab1_interfaces__srv__SetNoise_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    lab1_interfaces__srv__SetNoise_Response * data =
      (lab1_interfaces__srv__SetNoise_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    ...
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!lab1_interfaces__srv__SetNoise_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          lab1_interfaces__srv__SetNoise_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
